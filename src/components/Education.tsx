  ...
  return (
    <section id="education" className="py-24 px-6 max-w-6xl mx-auto">
      <div className="flex items-center gap-4 mb-16">
        ...
        <h2 className="text-4xl md:text-5xl font-extrabold tracking-tight text-slate-900">Education</h2>
      </div>

      <div className="space-y-8">
        {educations.map((edu, idx) => (
          <div key={idx} className="bg-white border text-left border-slate-200 rounded-3xl p-8 hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:-translate-y-1 hover:border-blue-300 transition-all duration-300">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
              ...
              <span className="bg-slate-100 text-slate-700 font-bold px-4 py-2 rounded-lg shrink-0 whitespace-nowrap">{edu.period}</span>
            </div>
            
            {edu.details.length > 0 && (
              <ul className="mt-4 space-y-2 pl-6 border-l-2 border-slate-100 text-slate-600">
