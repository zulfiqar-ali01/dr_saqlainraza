...
export default function Experience() {
  ...
  return (
    <section id="experience" className="py-24 px-6 bg-slate-50 dark:bg-slate-900 dark:bg-slate-100 border-t border-slate-100 dark:border-slate-800">
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-16 items-start">
        <div className="md:w-1/3 sticky top-32">
          <h2 className="text-4xl md:text-5xl font-extrabold tracking-tight text-slate-900 dark:text-white mb-6">Experience<span className="text-blue-600">.</span></h2>
        ...
        <div className="md:w-2/3 space-y-12 border-l-4 border-slate-200 dark:border-slate-700 pl-8 md:pl-12 ml-4 md:ml-0 relative">
          
          {experiences.map((exp, idx) => (
            <div key={idx} className="relative group">
              {/* Timeline marker node */}
              <div className="absolute -left-14 md:-left-[3.25rem] top-0 w-10 h-10 bg-white dark:bg-slate-900 dark:bg-slate-100 border-4 border-slate-200 dark:border-slate-700 rounded-full flex items-center justify-center group-hover:border-blue-500 duration-500 shadow-sm transition-all z-10">
                <div className="w-full h-full scale-[0.6] flex items-center justify-center">
                  {exp.icon}
                </div>
              </div>
              
              <div className="bg-white dark:bg-slate-900 dark:bg-slate-100 p-8 md:p-10 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 group-hover:shadow-xl group-hover:border-blue-100 transition-all duration-300">
                ...
                <h4 className="text-lg font-medium text-slate-500 dark:text-slate-400 mb-6">{exp.organization}</h4>
